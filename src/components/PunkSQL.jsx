...
function OnboardingScreen({ onComplete, lang }) {
  ...
  const ispt = lang === "pt";
  const slides = [
    { icon: ">_", title: ispt ? "BEM-VINDO AO PUNKSQL" : "WELCOME TO PUNKSQL", body: ispt ? "Resolva desafios SQL reais no navegador.\nEscreva queries · execute · suba de nível.\nSem configuração — só SQL." : "Solve real SQL challenges in-browser.\nWrite queries · run instantly · level up.\nNo setup needed — just SQL.", color: C.cyan },
    { icon: "◈", title: ispt ? "APRENDA" : "LEARN", body: ispt ? "10 módulos · 112 desafios\n\nSELECT · WHERE · ORDER BY\nGROUP BY · JOIN · Subqueries\nWindow Fns · CTEs · DML · DDL\n\nEASY → MED → HARD → EXPERT" : "10 modules · 112 challenges\n\nSELECT · WHERE · ORDER BY\nGROUP BY · JOIN · Subqueries\nWindow Fns · CTEs · DML · DDL\n\nEASY → MED → HARD → EXPERT", color: C.green },
    { icon: ">", title: ispt ? "CODE + QUIZ" : "CODE + QUIZ", body: ispt ? "[ CODE ]  112 desafios SQL\nEscreva SQL · RUN testa · SUBMIT valida\nBotões de keywords — sem teclado!\n\n[ QUIZ ]  60 questões\n4 opções cada · Timer de 15 segundos" : "[ CODE ]  112 SQL challenges\nWrite SQL · RUN to test · SUBMIT to check\nKeyword buttons — no keyboard needed!\n\n[ QUIZ ]  60 questions\n4 options each · 15-second timer", color: C.cyan },
    { icon: "◇", title: ispt ? "CARDS + XP" : "CARDS + XP", body: ispt ? "[ CARDS ]  Flashcards com swipe\nDireita = sabia    (+XP)\nEsquerda = errei   (-1 vida)\n3 vidas · Game Over reseta\n\n[ XP ]  Tudo dá XP\nSuba de nível · Ganhe badges" : "[ CARDS ]  Swipeable flashcards\nRight = knew it   (+XP)\nLeft  = missed    (-1 life)\n3 lives · Game Over resets\n\n[ XP ]  Everything earns XP\nLevel up · Unlock badges", color: C.amber },
    { icon: "▲", title: ispt ? "PRONTO PARA COMEÇAR?" : "READY TO START?", body: ispt ? "Botões SQL → insira keywords\nToque na tela → mova o cursor\nBotão ⌨ → abre/fecha teclado\n\nComece: módulo 1 — first_query\nBoa sorte, dev!" : "SQL buttons → insert keywords\nTap screen  → move cursor\n⌨ button   → toggle keyboard\n\nStart: module 1 — first_query\nGood luck, dev!", color: C.green },
  ];
  const s = slides[step];
