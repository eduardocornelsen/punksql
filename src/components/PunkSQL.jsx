...
const CHALLENGES_DB = [
  ...
  { id:91, mod:10, title:"create_products_archive", diff:"EASY", desc_en:"Create a table called products_archive with columns: id (INTEGER PRIMARY KEY), name (TEXT), category (TEXT), archived_at (TEXT).", desc_pt:"Crie uma tabela products_archive com: id (INTEGER PRIMARY KEY), name (TEXT), category (TEXT), archived_at (TEXT).", hint:"CREATE TABLE products_archive (id INTEGER PRIMARY KEY, name TEXT, ...)", validate:"CREATE TABLE products_archive (id INTEGER PRIMARY KEY, name TEXT, category TEXT, archived_at TEXT)", verify:"SELECT name FROM sqlite_master WHERE type='table' AND name='products_archive'", schema:"-- No existing table needed. Verify creation in sqlite_master." },
  ...
  { id:94, mod:10, title:"create_audit_log_table", diff:"MED", desc_en:"Create a table audit_log with: id (INTEGER PRIMARY KEY), action (TEXT NOT NULL), target_table (TEXT NOT NULL), changed_at (TEXT), user_id (INTEGER). Use NOT NULL constraints on critical fields.", desc_pt:"Crie a tabela audit_log: id (INTEGER PRIMARY KEY), action (TEXT NOT NULL), target_table (TEXT NOT NULL), changed_at (TEXT), user_id (INTEGER).", hint:"CREATE TABLE audit_log (id INTEGER PRIMARY KEY, action TEXT NOT NULL, ...)", validate:"CREATE TABLE audit_log (id INTEGER PRIMARY KEY, action TEXT NOT NULL, target_table TEXT NOT NULL, changed_at TEXT, user_id INTEGER)", verify:"SELECT name FROM sqlite_master WHERE type='table' AND name='audit_log'", schema:"-- No existing table needed." },
